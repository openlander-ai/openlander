...
export function IncidentTimeline({ events }: IncidentTimelineProps) {
  ...
  if (events.length === 0) {
    return (
      <div className="text-xs text-muted-foreground italic">{t('operations.noTimelineEvents')}</div>
    );
  }
      ...
      {groupedEvents.map((g, idx) => (
        <div key={idx} className="relative">
          ...
          <div className="flex flex-col">
            ...
            {g.count > 1 && (
              <span className="text-xs text-muted-foreground mt-2 italic">
