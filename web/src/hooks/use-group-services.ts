 ...
 *
 * Returns the list of deployable services attached to a group via the
 * canonical endpoint `GET /api/projects/:p/services` (P2 backend filters
 * to `kind NOT IN [postgres, mysql, redis, mongo, minio]`).
 *
 * This hook complements `useProjectsContext()` (which now exposes groups)
 * — when a consumer needs the deployable list under a specific group, it
