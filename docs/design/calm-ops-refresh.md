...
**원칙**: 메인 영역(사이드바 오른쪽)은 **full-width flex-1** 로 viewport를 꽉 채우고, **내부 콘텐츠 카드**에만 선택적으로 `max-w-8xl` (1360px) 적용. Dokploy의 실제 레이아웃이 이 방식이며, 전체 페이지를 `max-w-*`로 감싸서 가운데 정렬하면 헤더/탭 바가 viewport 안에서 떠 있는 어색함이 생긴다.

| 페이지 유형               | wrapper        | 내부 constraint                             |
| ------------------------- | -------------- | ------------------------------------------- |
| 대시보드 (Overview/Grid)  | **full-width** | 그리드가 자연스럽게 채움, `p-6 xl:p-8` 패딩 |
| 프로젝트/서비스/배포 상세 | **full-width** | 헤더/탭 바 viewport full, 패널은 자체 너비  |
| 설정 탭 내부 폼           | `max-w-xl`     | 가독성 위한 내부 제약                       |
| 로그인                    | `max-w-sm`     | 별도 (센터링)                               |
| 새 프로젝트 폼            | `max-w-xl`     | 별도 (폼)                                   |

Dokploy v0.18.4 스크린샷 (2026-04-22 확인) 에서 main content가 `flex-1` full-width로 채워지는 것을 검증함. 내부 특정 readability-critical form만 `max-w-*`로 제약.
...
| 2026-04-22 | 전체 페이지 wrapper에서 max-w-8xl 제거, main full-width | "전체 통일 1360px" 해석이 과도했음. Dokploy v0.18.4 대시보드 스크린샷에서 main content가 flex-1 full-width인 것 확인. 헤더/탭이 viewport 안에서 떠보이는 어색함 제거. max-w는 내부 form 같은 readability-critical 영역에만 적용 |
