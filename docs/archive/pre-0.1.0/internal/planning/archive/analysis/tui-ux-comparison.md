...
> **작성일**: 2026-02-26 (최종 업데이트: 2026-02-26 커밋 67bda82 기준)  
> **대상**: anomalyco/opencode TUI vs OpenLander TUI  
...
### OpenLander TUI 현황 (19개 컴포넌트 + 6개 온보딩 + 훅/유틸)

| 컴포넌트                 | 줄 수 | 역할                                      |
| ------------------------ | ----- | ----------------------------------------- |
| `App.tsx`                | 476   | 최상위 앱 (모드, 패널, 키보드, 디플로이)  |
| `Layout.tsx`             | 87    | Split-panel 레이아웃                      |
| `ChatPanel.tsx`          | 779   | 채팅 (입력, 메시지, 스트리밍, 슬래시피커) |
| `Prompt.tsx`             | 161   | OpenCode 스타일 프롬프트 (파이프 보더)    |
| `DashboardPanel.tsx`     | 400   | 시스템/프로젝트/활동 대시보드             |
| `ChatMessage.tsx`        | 267   | 메시지 렌더링 (15가지 타입)               |
| `AgentDisplay.tsx`       | 364   | 에이전트 액션 렌더링 (6종)                |
| `StatusBar.tsx`          | 107   | 하단 키바인드 + 상태 표시                 |
| `SlashCommandPicker.tsx` | 119   | 슬래시 명령 자동완성 드롭다운             |
| `ModelOverlay.tsx`       | 206   | 모델 선택 오버레이 (5 프로바이더)         |
| `ConnectOverlay.tsx`     | 209   | Git 프로바이더 연결 + 토큰 검증           |
| `RepoOverlay.tsx`        | 166   | 레포지토리 브라우저 + 원클릭 디플로이     |
| `HelpOverlay.tsx`        | 90    | 도움말 오버레이                           |
| `IMETextInput.tsx`       | 33    | CJK 텍스트 입력                           |
| `Spinner.tsx`            | 23    | 로딩 애니메이션                           |
| `ProgressBar.tsx`        | 58    | 진행률 바                                 |

**총 TUI 코드**: ~3,800줄 (컴포넌트) + ~1,900줄 (훅/유틸/커맨드/테마/온보딩)
...
| **오버레이**       | 다중 오버레이 (설정, 모델, 세션) | ✅ 4개 오버레이 (Help, Model, Connect, Repo) | ✅ **구현 완료** — OpenCode 수준으로 개선됨                             |
...
```typescript
// Prompt.tsx — OpenCode 스타일 프롬프트 (파이프 보더 + 에이전트/모델 표시)
// @opentui/solid <textarea> 기반, 1~6줄 멀티라인
// Enter = submit, Shift+Enter = newline
// onKeyDown → handlePromptKeyDown (picker 키보드 인터셉트)

// ChatPanel.tsx에서 관리:
- 히스토리 네비게이션 (↑↓, MAX_HISTORY = 100)
- 슬래시 명령 자동완성 (/ 입력 시 드롭다운 피커)
- 피커 키보드: ↑↓ 탐색, Enter 선택, Tab 완성, Esc 닫기
- Ctrl+L 채팅 클리어
- Ctrl+J 하단 점프
- PageUp/PageDown 수동 스크롤
```
...
| 항목            | 상태                              | 개선안                                               | 우선순위 |
| --------------- | --------------------------------- | ---------------------------------------------------- | -------- |
| **슬래시 명령** | ✅ 8개 Tier 1 명령, 드롭다운 피커 | ✅ **구현 완료** — ↑↓/Enter/Tab/Esc 키보드 완전 지원 |
| **히스토리**    | ✅ ↑↓ + 100개 캐시                | ✅                                                   |
| **CJK/IME**     | ✅ 잘 구현됨                      | ✅ 한/중/일 지원. 글로벌 대응 OK                     |
| **@멘션**       | ❌                                | 🟡 @프로젝트명 으로 특정 프로젝트 참조 (v0.6+)       |
| **파일 첨부**   | ❌                                | ❌ 배포 에이전트에 불필요                            |
| **멀티라인**    | ✅ Shift+Enter                    | ✅ **구현 완료** — Prompt.tsx에서 1~6줄 지원         |
...
```typescript
// App.tsx — useKeyboard 핸들러 (evt.name 기반 — @opentui/core KeyEvent)
// Tab: 패널 전환
// ?: 도움말
// q: 종료 (대시보드 패널 포커스 시)
// Ctrl+C: 두 번 눌러 종료
// Esc: 오버레이 닫기 (Model, Connect, Repo, Help)
//
// ChatPanel.tsx — handlePromptKeyDown
// Ctrl+L: 채팅 클리어
// Ctrl+J: 하단 점프
// ↑↓: 히스토리/피커 네비게이션
// /: 슬래시 명령 모드 (드롭다운 피커 표시)
// PageUp/PageDown: 수동 스크롤
//
// ✨ 모든 핸들러는 evt.name 사용 (evt.key/evt.char는 KeyEvent에 존재하지 않음)
```
...
| **현재 키바인드** | ✅ 직관적          | ✅ Tab, ?, Ctrl+C 등 표준적. 키보드 버그 수정 완료 (evt.name) |
...
#### OpenLander (재구성 완료 — 3계층 시스템)

```
Tier 1 — 직접 실행 (LLM 미경유, 오버레이/직접 액션):
/help      — 도움말 (단축키 목록)
/clear     — 채팅 클리어
/model     — 모델 선택 오버레이 (5 프로바이더 13개 모델)
/compact   — 컨텍스트 압축 (요약 프롬프트)
/connect   — Git 프로바이더 연결 + 토큰 검증 오버레이
/repo      — 레포지토리 브라우저 + 원클릭 디플로이 (실시간 빌드 진행)
/exit      — 종료
/theme     — 테마 변경 (예약)

Tier 2 — 직접 실행 (미구현, 계획):
/logs, /stop, /start, /deploy 등 배포 직접 실행 명령

Tier 3 — 제거됨:
에이전트 프록시 명령은 의미 없어서 전부 제거
슬래시 명령 = LLM 안 거치고 직접 실행이 원칙
```

### 📌 슬래시 명령 평가

> **기존 22개 → 8개 Tier 1으로 재구성.** Tier 3 에이전트 프록시 명령 전량 제거.
> `슬래시 명령은 LLM 안 거치고 직접 실행이 원칙` 구현 완료.
>
> ✅ 구현 완료: `/model`, `/compact`, `/connect`, `/repo`
> 🟡 예정: Tier 2 직접 실행 명령 (`/logs`, `/stop`, `/start`)
...
| #   | 개선                | 구현 방법                        | 예상 공수 | 상태   |
| --- | ------------------- | -------------------------------- | --------- | ------ |
| 10  | **Tier 2 슬래시**   | /logs, /stop, /start 직접 실행   | 1일       | 미구현 |
| 11  | **에이전트 전환**   | deploy↔diagnose↔monitor Tab 전환 | 1일       | 미구현 |
| 12  | **테마 프리셋**     | catppuccin, dracula, nord 등     | 0.5일     | 미구현 |
| 13  | **커스텀 키바인드** | openlander.json에서 키 변경      | 1일       | 미구현 |
| 14  | **Leader key**      | Ctrl+K → 복합 액션               | 0.5일     | 미구현 |
| 15  | **설정 오버레이**   | 인앱에서 설정 변경               | 1일       | 미구현 |
...
| 항목                    | 현재                    | 개선안                                              |
| ----------------------- | ----------------------- | --------------------------------------------------- |
| **ChatPanel 크기**      | 779줄 (과대)            | 메시지 표시 + 피커 + 스크롤 로직을 별도 훅으로 추출 |
| **DashboardPanel 크기** | 400줄 (대형)            | fetchAll 로직을 useProjects/useSystem 훅으로 분리   |
| **handleStreamEvent**   | ChatPanel에 인라인      | useChat 훅 활용 (이미 존재하지만 미사용)            |
| **키바인드 산재**       | App.tsx + ChatPanel.tsx | 중앙 키바인드 레지스트리 (현재 evt.name 통일 완료)  |
...
3. ~~**`/model` 런타임 변경**~~ ✅ 구현 완료 → **Tier 2 슬래시 명령** (`/logs`, `/stop`, `/start` 직접 실행)
