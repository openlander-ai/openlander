...
- [x] 관리 컨테이너의 크래시, 반복 재시작, 리소스 포화를 감지한다
- [x] 호스트 디스크 부족(5GB 미만)을 감지한다
- [ ] 외부 컨테이너 추가/제거를 감지한다 (v0.0.9 `listAllContainers` 활용)
- [x] 동일 이슈 반복 알림 방지 (한 번 알린 건 해결 전까지 재알림 안 함)
- [x] 시간당 최대 3건 제한
- [x] 사용자가 알림을 dismiss할 수 있다
- [ ] 프로젝트 관련 이상 징후는 해당 프로젝트 Timeline에 표시된다
- [x] 시스템 전체 알림은 헤더 알림 인디케이터 + 드롭다운 패널에 표시된다
- [x] 알림 메시지에 **액션 버튼** 포함 ("로그 볼까?", "정리할까?")
