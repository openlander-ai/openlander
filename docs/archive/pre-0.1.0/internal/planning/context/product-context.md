...
핵심 차별점: MCP 프로토콜로 IDE 에이전트와 직접 통합. 64개 도구로 배포/관리/모니터링 자동화. 빌드 실패 시 AI가 자동으로 분석하고 복구 (백엔드에서 조용히). 1차 타겟: 배포 자동화를 원하는 DevOps/백엔드 엔지니어 (DEC-008, DEC-037).
...
| 채널              | 파일                                                 | 사용자                                | 특징                                                          |
| ----------------- | ---------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------- |
| **MCP**           | `src/mcp/server.ts`                                  | IDE 에이전트 (Cursor, Claude Code 등) | **메인 인터페이스.** 64개 도구를 MCP 프로토콜로 노출          |
