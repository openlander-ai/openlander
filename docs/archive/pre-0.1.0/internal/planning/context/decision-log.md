...
### DEC-042: 챗봇 → 스마트 디플로이 터미널 전환

**날짜**: 2026-03
**결정**: DEC-041에서 계획한 챗봇 패널을 도입하지 않고, 배포 터미널 안에서 AI가 인라인으로 개입하는 "스마트 디플로이 터미널" 방식으로 전환.
**거부한 대안**: 챗봇 패널 (화면 우측/하단 슬라이드, 멀티턴 대화)
**이유**:

- Vercel 분석: Vercel은 배포 플로우에 AI를 넣지 않음. OpenLander의 차별점은 "배포 중에 AI가 함께 있다"는 것인데, 챗봇은 배포 플로우 밖의 별도 인터페이스 → 차별점이 희석됨
- 터미널 인라인 방식이 더 자연스러움: 배포 이벤트 스트림에 AI 이벤트를 추가하는 것으로 구현 단순
- 챗봇은 세션 관리, 대화 히스토리, 멀티턴 상태 등 복잡도가 높음 (DEC-043 참조)
- MCP-only 사용자(코딩 에이전트)와 웹 사용자 모두에게 일관된 경험: 터미널 스트림이 공통 언어

**DEC-041 UPDATE**: 챗봇 도입 결정을 철회. 버튼→챗봇 연결 대신 버튼→터미널 AI 인라인으로 전환.
**재검토 조건**: 사용자가 멀티턴 대화 기반 배포 제어를 명시적으로 요청할 때

---

### DEC-043: 세션 DB 제거 — 휘발성 전환

**날짜**: 2026-03
**결정**: 웹 AI 인터랙션에 세션 DB를 두지 않는다. 배포 이벤트는 배포 히스토리 DB에 이미 저장되므로 별도 AI 세션 테이블 불필요.
**거부한 대안**: `ai_sessions` 테이블 신설 (대화 히스토리, 세션 상태 저장)
**이유**:

- 챗봇 패턴을 버리면서 멀티턴 대화 상태를 유지할 이유가 없어짐
- 배포 이벤트(진단/처방)는 이미 배포 히스토리에 연결됨 — 중복 저장 불필요
- DB 스키마 단순화 → 유지보수 비용 감소
- 1인 메인테이너 리소스 제약 — 세션 만료, 정리 로직 등 부가 작업 제거

**재검토 조건**: 멀티턴 AI 대화 기능을 도입할 때

---

### DEC-044: 자동 복구 1회 제한 — UI 레벨

**날짜**: 2026-03
**결정**: 배포 실패 시 AI 자동 복구를 UI 레벨에서 1회만 허용. 2회 이상 시도하지 않음.
**거부한 대안**: 최대 3회 재시도 (DEC-026 원래 설계)
**이유**:

- 복구 실패가 반복되면 사용자 혼란 증가 — "AI가 계속 뭔가 하는데 왜 안 되지?"
- 1회 복구 후 실패하면 사용자에게 명확한 에러 상태를 보여주는 것이 더 정직함
- 무한 루프 방지 — 동일 에러 반복 감지 로직 없이도 안전
- 복잡한 복구 시나리오는 MCP 경유(코딩 에이전트)가 더 적합

**DEC-026 UPDATE**: 최대 3회 재시도 → UI 레벨 1회 제한으로 축소.
**재검토 조건**: 사용자 피드백에서 "1회 복구로 부족하다"는 케이스가 반복될 때

---

### DEC-045: LLM 인증 — v1.0.0은 API key만, OAuth 추후

**날짜**: 2026-03
**결정**: v1.0.0에서 LLM 인증은 API key 직접 입력(BYOK)만 지원. Provider OAuth(OpenAI/Anthropic/OpenRouter/Google 계정 로그인)는 v1.0.0 이후로 연기.
**거부한 대안**: OAuth 포함 릴리즈 (DEC-020에서 계획했던 v0.0.12 스펙)
**이유**:

- 제품 차별점이 AI인데, API key 없으면 Coolify 하위호환 — key 등록이 핵심 온보딩 단계
- 모든 인증 방식을 동등하게 지원하되, 하나를 특별히 밀지 않음 (특정 프로바이더 종속 방지)
- OAuth는 각 프로바이더별 별도 연동 필요 (Client ID, 엔드포인트, 토큰 플로우 모두 다름) — v1.0.0 스코프 초과
- BYOK가 베이스라인으로 충분: Gemini 무료 티어, OpenRouter 무료 티어 등 key 발급 장벽 낮음

**DEC-013 UPDATE**: Provider OAuth 연기 결정 유지. v1.0.0 이후 재평가.
**재검토 조건**: BYOK 온보딩 이탈률이 높을 때, 또는 특정 프로바이더 OAuth 요청이 반복될 때

---

### DEC-046: AI 버튼 분류 — 16개 중 4개만 AI, 12개 직접

**날짜**: 2026-03
**결정**: 웹 UI 액션 버튼 16개 중 4개(Deploy, Redeploy, Map Domain, Rollback)만 AI 경유. 나머지 12개는 직접 API 호출. AI 버튼은 스파클 아이콘으로 시각적 구분.
**거부한 대안**: 모든 버튼을 AI 경유 (DEC-041 원래 계획)
**이유**:

- 모든 버튼을 AI 경유하면 API key 없는 사용자가 기본 기능도 못 씀 — 진입 장벽 과도
- Stop, Restart, Delete, 환경변수 CRUD 등은 단순 API 호출로 충분 — AI 개입이 오히려 느리고 불필요
- AI가 의미 있는 버튼만 선별: 배포 관련(Deploy/Redeploy), 복잡한 후속 작업이 있는 것(Map Domain), 위험한 작업(Rollback)
- 스파클 아이콘으로 "이 버튼은 AI가 처리한다"는 시각적 신호 제공 → 사용자 기대치 관리

**DEC-041 UPDATE**: 모든 버튼 AI 경유 → 4개 AI 버튼 + 12개 직접 버튼으로 분류.
**재검토 조건**: AI 버튼 분류가 사용자 혼란을 유발하거나, 추가 버튼의 AI 경유 요청이 반복될 때
