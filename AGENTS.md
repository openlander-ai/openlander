...
Tool definition files back the MCP tool system. The MCP adapter exposes **5 composite tools** (`openlander_deploy|_project|_service|_managed_service|_monitor`) over 98 unique default operations, plus **13 platform tools** gated by `config.mcp.platformTools`. The registry currently spans (130 ToolDefs, 25 tool definition files), plus 31 ToolDefs generated from Application Operation contracts. Two adapters exist:

- Current MCP-facing snapshot: 98 default MCP ToolDefs + 13 optional platform ToolDefs. Keep `test/mcp/*registry*` authoritative when counts change.
