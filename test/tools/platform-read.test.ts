  ...
  it('platform_config redacts all required secret fields', async () => {
    ...
    const result = (await getTool('platform_config').execute(
      ...
      llm: {
        apiKey: string;
        authToken: string;
        providers: Record<string, { apiKey: string; authToken: string; provider: string }>;
      };
    ...
    expect(result.llm.providers['zai-coding']?.apiKey).toBe('***REDACTED***');
    expect(result.llm.providers['zai-coding']?.authToken).toBe('***REDACTED***');
    expect(result.llm.providers['zai-coding']?.provider).toBe('openai-compatible');
