...
describe('App Routes', () => {
  ...
  it('redirects the retired top-level deployments page to Activity deploy filter', async () => {
    const fs = await import('node:fs/promises');
    const retiredPagePath = join(__dirname, '..', '..', 'web', 'src', 'pages', 'DeploymentsList.tsx');
    const content = await fs.readFile(
      join(__dirname, '..', '..', 'web', 'src', 'App.tsx'),
      'utf-8',
    );
    ...
    expect(content).toContain('to="/activity?type=deploy"');
    expect(content).not.toContain('<DeploymentsList />');
    await expect(fs.access(retiredPagePath)).rejects.toThrow();
  });
});
