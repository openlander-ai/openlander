...
// Legacy SQLite-era monolith. The current route contract is covered by focused
// suites under test/web/api/* and test/api/*. This file still documents old
// expectations for removed project-level runtime actions and built-in AI flow,
// so running it against the Postgres/service-owned model produces false
// failures. Keep skipped until the remaining useful cases are migrated into
// focused route suites.
describe.skip('Web API Routes — legacy monolith superseded by focused route suites', () => {
  let app: Hono;
  let db: Database;
  let tmpDir: string;
  let ctx: AppContext;
  ...
  it('POST /api/projects/deploy keeps explicit failure when user selects cancel after failure analysis', async () => {
    (
  ...
  // ---------------------------------------------------------------------------
  // Git provider auto-deploy webhooks are disabled in 0.1.
  // ---------------------------------------------------------------------------

  it.each([
    ['GET', '/api/projects/p1/webhooks'],
    ['POST', '/api/projects/p1/webhooks'],
    ['DELETE', '/api/projects/p1/webhooks/github'],
  ])('%s %s returns FEATURE_DISABLED', async (method, path) => {
    db.createProject({ id: 'p1', name: 'my-app', repoUrl: 'https://github.com/user/app' });
