...
export const gitToolDefs: ToolDef[] = [
  ...
  },
  {
    ...
    riskLevel: 'low',
    description:
      'Clone a repo and scan for all Dockerfiles. Returns paths like ["Dockerfile", "frontend/Dockerfile", "backend/Dockerfile"]. If multiple are found and no valid Compose deployment is selected, create one Application plan per chosen dockerfile_path; repeat with the same target_project_id to attach sibling Applications. Errors: CLONE_FAILED.',
    mcpDescription:
      'Scan a repository for Dockerfiles. Multiple candidates require one dockerfile_path and one Application per deploy plan.',
