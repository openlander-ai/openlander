...
export async function preflightCheck(
  ...
  try {
    ...
    const nameCheck: PreflightCheck = existingContainer
      ? {
          pass: false,
          detail: `Container "${containerName}" already exists (${existingContainer.managedByOpenLander ? 'managed' : 'external'}, ${existingContainer.state}). If this is an existing OpenLander service, use openlander_service.deploy_service to redeploy it. Otherwise remove the conflicting container first.`,
        }
      : {
          pass: true,
