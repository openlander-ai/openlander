...
export async function persistDeployConfig(params: {
  ...
  if (params.config._serviceId) {
    await params.db.saveDeployConfigForService(params.config._serviceId, json, CONFIG_VERSION);
    return;
  }
...
export async function loadResourceLimitsForProject(
  ...
  return resourceLimitsFromConfigRow(configRow);
}

export async function loadResourceLimitsForService(
  db: Database,
  serviceId: string,
): Promise<ResourceLimitConfig | null> {
  const configRow = await db.loadDeployConfigForService(serviceId);
  return resourceLimitsFromConfigRow(configRow);
}

export async function loadResourceLimitsForDeployTarget(
  db: Database,
  params: { projectId: string; serviceId?: string | null },
): Promise<ResourceLimitConfig | null> {
  if (params.serviceId) {
    return loadResourceLimitsForService(db, params.serviceId);
  }
  return loadResourceLimitsForProject(db, params.projectId);
}

function resourceLimitsFromConfigRow(configRow: Awaited<ReturnType<Database['loadDeployConfig']>>) {
