...
import { DeploymentNotActiveError } from '../../errors.js';
import {
...
export function registerDeployLogStreamRoutes(api: Hono, ctx: AppContext): void {
  api.post('/deployments/:id/cancel', async (c) => {
    const id = c.req.param('id');
    const resolved = await resolveDeploymentLogSource(ctx, id);
    if (!resolved) {
      return c.json({ error: 'NOT_FOUND', message: 'Deployment not found' }, 404);
    }
    if (resolved.deployLog) {
      const err = new DeploymentNotActiveError(id);
      return c.json(err.toJSON(), 409);
    }

    const cancelled = ctx.docker.cancelBuild(resolved.project.id);
    if (!cancelled) {
      const err = new DeploymentNotActiveError(id);
      return c.json(err.toJSON(), 409);
    }
    ...
    return stream(c, async (s) => {
      ...
      unsubscribers.push(
        eventBus.on('build:output', (payload) => {
          ...
          void emitLine({
            phase: normalizeDeployLogPhase(payload.phase),
