...
export function createDeployableServiceRoutes(ctx: AppContext): Hono {
  ...
  api.get('/projects/:id/services', async (c) => {
    ...
    const composeParent = visibleDeployables.find((service) => service.kind === 'compose');
    ...
    const [domainMappingsByService, credentials, lastDeploys, storedComposeConfig] =
      await Promise.all([
        loadDomainMappingsByService(ctx, visibleDeployables),
        gitCredentialManager ? gitCredentialManager.list() : Promise.resolve([]),
        lastDeploysPromise,
        composeParent && typeof ctx.db.loadDeployConfigForService === 'function'
          ? ctx.db.loadDeployConfigForService(composeParent.id)
    ...
    const trafficTargetId = storedTrafficService
      ? composeChildren.find(
          (service) => service.name === `${project.name}/${storedTrafficService}`,
        )?.id
    ...
    const aggregateStatus = aggregateComposeStatus(
      ...
      composeChildren.map((service) => service.id),
    );
  ...
  api.get('/projects/:p/services/:s', async (c) => {
    ...
    const [environments, deployLogs, serviceRecords, domainMappingsByService, gitCredential] =
      await Promise.all([
        ctx.db.getEnvironmentsByProject(project.id),
        service && typeof ctx.db.getDeployLogsForService === 'function'
