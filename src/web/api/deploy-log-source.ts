...
export const DEPLOY_LOG_PHASES = [
  'clone',
  'image_pull',
  'build',
  'container_create',
...
export type DeployLogPhase = (typeof DEPLOY_LOG_PHASES)[number];
...
/**
 * Normalize old and live deployment phase labels onto the frontend
 * PhaseRail contract.
 */
export function normalizeDeployLogPhase(phase: string | null | undefined): DeployLogPhase {
  const key =
    phase
      ?.trim()
      .toLowerCase()
      .replace(/[-\s]+/g, '_') ?? '';
  if (key === 'clone') return 'clone';
  if (key === 'pull' || key === 'image' || key === 'image_pull') return 'image_pull';
  if (key === 'create' || key === 'container_create') return 'container_create';
  if (key === 'tag' || key === 'run' || key === 'start' || key === 'container_start') {
  ...
  if (
    ...
    key === 'connectivity' ||
    key === 'healthcheck' ||
    key === 'healthcheck_wait'
  ) {
    return 'healthcheck_wait';
  }
  return 'build';
}

function parseBuildLogLine(line: string): DeployLogLineEvent {
  ...
  const rawPrefix = bracket[1] ?? 'info';
  return {
    phase: normalizeDeployLogPhase(rawPrefix),
    prefix: rawPrefix.trim().toLowerCase() || 'info',
    payload: bracket[2] ?? '',
  };
}
