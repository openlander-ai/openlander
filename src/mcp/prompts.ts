...
5. **Monitor** — Behavior depends on the path. A **new Application** \`deploy_app\` blocks until terminal by default (\`wait: true\`) and returns the final result; pass \`wait: false\` to return immediately. A successful \`deploy_app\` records an implicit immutable Release from the deployed digest without rebuilding. But when \`deploy_app\` resolves to an **existing** Application/Compose workload (\`service_id\`/\`service_name\`, or a single-workload \`name\`) it delegates to \`update_app\` and returns \`deploying\` immediately — non-blocking, like \`execute_deploy_plan\`. Stateful Compose changes return \`pending_approval\` with a secret-free diff and \`poll_call\`; do not retry while approval is pending. For every non-blocking path, poll \`get_deploy_status\` until terminal. During a build, \`get_build_log\` returns \`complete:false\`, the latest step, and recent output; after completion it returns the full persisted output.
...
- Saved Compose env is interpolation input only. Each child receives only keys declared in its \`environment\` or \`env_file\`; use \`environment: {}\` to explicitly inject none. A service with saved env and no declaration is rejected with \`COMPOSE_ENV_DECLARATION_REQUIRED\`.
...
2. **Gather evidence (all read-only)**:
    - \`get_build_log\` — clone / image / build evidence. An active service/project target returns \`complete:false\`, current step, and the latest 30 lines; after completion pass \`deploy_id\`, or \`project_name\` + \`deploy_index\` (0 = latest), for the full persisted output.
    - \`get_logs\` — runtime crashes that happen *after* the container started.
    - \`diagnose_service\` — combined health + recent logs for an Application/Compose workload. Prefer \`service_id\`; pass \`path\` to probe a specific route. Dependency checks use the running container's actual env; saved env is masked drift evidence only.
...
- Project/Application hard-delete aliases such as \`delete_app\`, \`delete_project\`, and \`purge_project\` are **human-UI-only** and return \`HUMAN_UI_ONLY\`. Database/Cache/Storage deletion actions such as \`remove_service\` follow the effective Security permission and may execute, wait for approval, or return \`OPERATION_PERMISSION_DENIED\`. Host-wide \`cleanup_docker\` follows the global destructive permission only. Do **not** substitute \`remove_service\` or \`cleanup_docker\` for Application/Project cleanup (those target Database/Cache/Storage resources or Docker hosts, not Applications).
- \`archive_project\` / \`unarchive_project\` are the MCP-safe soft lifecycle path for a whole Project. \`archive_service\` / \`unarchive_service\` target one Application/worker. All four enter the **human approval queue** before executing — follow the returned \`poll_call\` or poll \`mcp_action_status\` with \`action_run_id\`. Archive is reversible cleanup, not permanent deletion: archived Applications are hidden from default active lists but can be inspected with \`list_archived_services\` and restored with \`unarchive_service\`. Preserved Stateful Compose resources resume in place; other restored workloads do not redeploy automatically.
- \`deploy_app(target_project_id=...)\` can add an Application, worker, or Compose workload into an existing Project. It uses that Project's network from the first container start, then transactionally attaches the workload after success. It returns \`target_project_id\`, \`runtime_project_id\`, and \`service_id\`; use the returned compatibility \`service_id\` for follow-up workload actions. Do not combine it with \`expose=true\`; expose after attach if needed.
- Repository selection is deterministic: valid Compose is the default; otherwise one Dockerfile creates one Application per plan. If multiple Dockerfiles exist, choose a returned \`candidate_dockerfiles\` entry with \`dockerfile_path\`, then repeat with the same \`target_project_id\` for additional Applications. \`prefer_dockerfile=true\` overrides Compose and selects the root Dockerfile when present.
