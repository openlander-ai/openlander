...
- \`remove_service\`, \`delete_app\`, \`archive_*\`, \`purge_project\` are **human-UI-only** and return \`HUMAN_UI_ONLY\` / \`OPERATION_REQUIRES_HUMAN_UI\`. Do **not** substitute \`remove_service\` or \`cleanup_docker\` (those target managed infrastructure, not deployable apps). Surface the web-UI Danger-zone path to the user instead.
- Destructive actions that remain exposed (e.g. \`bulk_delete_env_vars confirm=true\`) enter the **human approval queue** before executing — poll \`mcp_action_status\` with the returned \`action_run_id\`.
