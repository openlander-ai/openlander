...
- Use \`stop_app\` / \`delete_app\` for Applications; \`delete_app\` retains volumes. When the user explicitly asks to allow cleanup for a Project, use \`set_project_permissions\` with \`app_lifecycle="allow"\`, and include action_run_ids for the specific pending requests to resume. Use cleanup_apps for multiple apps and poll mcp_action_status for per-service results. Never re-submit a pending request. Never infer authorization from a blocked action. Database/Cache/Storage still use \`stop_service\` / \`remove_service\`. Whole-Project hard delete and host purge retain their existing human controls.
- \`archive_project\` / \`unarchive_project\` are the MCP-safe soft lifecycle path for a whole Project. \`archive_service\` / \`unarchive_service\` target one Application/worker. All four follow the Project permission: an explicit allow executes immediately; otherwise they enter the **human approval queue** — follow the returned \`poll_call\` or poll \`mcp_action_status\` with \`action_run_id\`. Archive is reversible cleanup, not permanent deletion: archived Applications are hidden from default active lists but can be inspected with \`list_archived_services\` and restored with \`unarchive_service\`. Preserved Stateful Compose resources resume in place; other restored workloads do not redeploy automatically.
