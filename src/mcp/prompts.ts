...
- Use \`stop_app\` / \`delete_app\` for Applications; \`delete_app\` retains volumes. When the user explicitly asks to allow cleanup for a Project, use \`set_project_permissions\` with \`destructive_actions="allow"\`, then continue without web approval. Never infer authorization from a blocked action. Database/Cache/Storage still use \`stop_service\` / \`remove_service\`. Whole-Project hard delete and host purge retain their existing human controls.
