...
Project/app hard delete and purge are intentionally NOT exposed as MCP actions. If the user asks to delete, remove, or purge a project or app, tell them to use the web UI: Settings → Danger zone for that project/service. For deployable app cleanup, use archive_service and wait for human approval. Do NOT substitute remove_service or cleanup_docker — those target managed infrastructure services and Docker hosts, not deployable apps.`;
