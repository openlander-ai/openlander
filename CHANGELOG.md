...
- Keep retry-safe Cloudflare operations provisioning through longer Colima
  outbound interruptions instead of requiring a manual retry or VM restart.

## [0.3.6-rc.6] - 2026-08-11
...
- Retry transient Cloudflare network failures for idempotent API operations
  without repeating DNS or Tunnel creation requests.
- Preserve the original publish failure when remote cleanup cannot be verified
  and show server-to-Cloudflare connectivity guidance instead of blaming the
  Application.

## [0.3.6-rc.3] - 2026-08-11

### Fixed

- Retry one transient OAuth token endpoint network failure before marking a
  Cloudflare connection as unavailable during startup reconciliation.

## [0.3.6-rc.2] - 2026-08-11
