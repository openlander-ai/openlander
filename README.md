...
| **v1.0.0-rc.8** | Health Monitoring & Ops UI   | RC     | Health check foundation (strategies, profiles, probes), ProjectHealthMonitor + ContainerStateReconciler, Operations center (Overview, Deployments, Recovery tabs), UI polish & a11y  |
| **v1.0.0**      | Stable Release               | TBD    | RecoveryCoordinator (7-condition eligibility gate), multi-provider LLM (8 LLM providers), approval gates for high-risk recovery, AI usage dashboard, operations center, 99 MCP tools |
...
Once connected, AI agents get 99 MCP tools including:
